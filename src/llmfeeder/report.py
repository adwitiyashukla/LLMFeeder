...
from llmfeeder.models import CheckResult, Document, Verdict
...
def render_report(result: CheckResult, *, title: str = "LLMFeeder report") -> str:
...
<footer>
  Generated by <a href="https://github.com/adwitiyashukla/LLMFeeder">LLMFeeder</a>.
  Every highlight is an exact character range in the source file, not a paraphrase.
</footer>
<script type="application/json" id="llmfeeder-data">{payload}</script>
<script>{_JS}</script>
</body>
</html>
"""
...
def write_report(
    result: CheckResult,
    path: str | Path = "llmfeeder-report.html",
    *,
    title: str = "LLMFeeder report",
) -> Path:
