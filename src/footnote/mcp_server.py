...
_SOURCES_DESCRIPTION = (
    "Check whether every claim in the text is supported by the documents in a folder. "
    ...
    "text is the passage to verify, sources_dir is the folder of source documents, "
    "and judge is lexical for the offline deterministic judge, llm to adjudicate with "
    "a language model, or auto to use the LLM when a key is present."
)
...
_TEXT_DESCRIPTION = (
    ...
    "text is the passage to verify, sources are the source documents as plain strings, "
    "and judge is lexical, llm, or auto."
)
...
def build_server(default_sources: str = ".") -> Any:
    ...
    @server.tool(description=_TEXT_DESCRIPTION)
...
def serve(default_sources: str = ".") -> None:
